...
class ReadEraClone(ctk.CTk):
                ...
                while not self.stop_workers.is_set():
                    ...
                    try:
                        ...
                        if img:
                            ...
                            # Schedule UI update in main thread - CTkImage must be created in main thread
                            self.after(0, lambda i=index, image=img, ht=h_target, wt=w_target: self._update_page_ui(i, image, ht, wt))
    ...
    def _update_page_ui(self, index, pil_img, h_target, w_target):
    ...
    def _init_reader_ui(self):
        ...
        # Scroll Events
        self.reader_scroll._parent_canvas.bind("<Configure>", lambda e: self.check_visibility(), add="+")
        self.reader_scroll._parent_canvas.bind("<MouseWheel>", self._on_scroll, add="+")
        self.reader_scroll._parent_canvas.bind("<Button-4>", self._on_scroll, add="+")
        self.reader_scroll._parent_canvas.bind("<Button-5>", self._on_scroll, add="+")
    ...
    def _save_progress(self):
        if self.current_book_id and self.page_labels:
            canvas = self.reader_scroll._parent_canvas
    ...
    def _jump_to_page(self, index):
        if index < len(self.page_labels):
            ...
            y = lbl.winfo_y()
            total_h = self.reader_scroll._parent_canvas.bbox("all")[3]
            if total_h > 0:
                fraction = y / total_h
                self.reader_scroll._parent_canvas.yview_moveto(fraction)
